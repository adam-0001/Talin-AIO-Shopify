...
func PreloadCheckout(t *shopifyTasks.ShopifyTask) {
	...
	var preloadVariant string
	for {
		if !t.IsRunning() {
			return
		}
		preloadVariant, err = steps.PreloadFetchVariant(t)
		if err != nil {
			log.Println("Error Finding Preload Item:", err)
			t.SetStatus("Error Finding Preload Item - Retrying...", colors.Red)
			time.Sleep(t.Settings.Shopify.ErrorDelay)
		} else {
			break
		}
	...
	for {
		...
		err = steps.PreloadAtc(t, preloadVariant)
		if err != nil {
			if err == errs.ErrorWithRequest {
				errs.HandleErrorStatus(t, err)
			} else {
				t.SetStatus("Retrying Preload ATC", colors.Yellow)
			}
			time.Sleep(t.Settings.Shopify.ErrorDelay)
		} else {
			break
	...
	err = steps.PreloadSetShippingRate(t)
	if err != nil {
		return //Can assume this is a stop task b/c it is the only possible return val
	}
	t.SetStatus("Preload Acquired", colors.Yellow)
	err = func() error {
		for {
			if !t.IsRunning() {
