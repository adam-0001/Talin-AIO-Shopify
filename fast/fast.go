...
//SetShipping
//findApiToken
func FastCheckout(t *shopifyTasks.ShopifyTask) {
	...
	steps.FastFindApiToken(t, t.LastResponse.Text)
	var tmp = false
	for {
		if !t.IsRunning() {
			return
	...
	if err != nil {
		...
		t.End("Stopped - Error Handling Queue!")
	}
	steps.FastFindApiToken(t, t.LastResponse.Text)
	...
	tmp = false
	for {
		err = steps.FastSetDeliveryInfo(t, tmp)
		if err == nil {
			break
		}
		tmp = true
		if err == errs.ErrorTaskStop {
			return
		}
		errs.HandleErrorStatus(t, err)
	...
	err = steps.FastSetShippingRate(t)
	if err != nil {
